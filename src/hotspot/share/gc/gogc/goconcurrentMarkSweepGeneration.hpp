#ifndef SHARE_GC_gogc_goConcurrentMarkSweepGeneration_HPP
#define SHARE_GC_gogc_goConcurrentMarkSweepGeneration_HPP

#include "gc/gogc/gogcOopClosures.hpp"
#include "gc/gogc/gSpaceCounters.hpp"
#include "gc/gogc/yieldingWorkgroup.hpp"
...
class gogcGCAdaptivePolicyCounters;
class gogcTracer;
class ConcurrentGCTimer;
class goConcurrentMarkSweepGeneration;
class goConcurrentMarkSweepPolicy;
class goConcurrentMarkSweepThread;
...
class gogcBitMap {
 ...
 public:
  // constructor
  gogcBitMap(int shifter, int mutex_rank, const char* mutex_name);
...
// Represents a marking stack used by the gogc collector.
// Ideally this should be GrowableArray<> just like MSC's marking stack(s).
class gogcMarkStack: public CHeapObj<mtGC>  {
  friend class gogcCollector;   // To get at expansion stats further below.
 ...
 public:
  gogcMarkStack():
    _par_lock(Mutex::event, "gogcMarkStack._par_lock", true,
...
class CardTableRS;
class gogcParGCThreadState;

class ModUnionClosure: public MemRegionClosure {
 protected:
  gogcBitMap* _t;
 public:
  ModUnionClosure(gogcBitMap* t): _t(t) { }
  void do_MemRegion(MemRegion mr);
};

class ModUnionClosurePar: public ModUnionClosure {
 public:
  ModUnionClosurePar(gogcBitMap* t): ModUnionClosure(t) { }
  void do_MemRegion(MemRegion mr);
};
...
class ChunkArray: public CHeapObj<mtGC> {
 ...
 public:
  ...
  void reset() {
    _index = 0;
    if (_overflows > 0) {
      log_trace(gc)("gogc: ChunkArray[" SIZE_FORMAT "] overflowed " SIZE_FORMAT " times", _capacity, _overflows);
    }
    _overflows = 0;
  }
...
class gogcStats {
 private:
  goConcurrentMarkSweepGeneration* const _gogc_gen;   // The gogc (old) gen.

  unsigned int _saved_alpha; // 0-100
  unsigned int _gc0_alpha;
  unsigned int _gogc_alpha;

  double _gc0_duration;
  double _gc0_period;
  size_t _gc0_promoted;         // bytes promoted per gc0
  double _gogc_duration;
  double _gogc_duration_pre_sweep; // time from initiation to start of sweep
  double _gogc_period;
  size_t _gogc_allocated;        // bytes of direct allocation per gc0 period

  // Timers.
  elapsedTimer _gogc_timer;
  TimeStamp    _gc0_begin_time;
  TimeStamp    _gogc_begin_time;
  TimeStamp    _gogc_end_time;

  // Snapshots of the amount used in the gogc generation.
  size_t _gogc_used_at_gc0_begin;
  size_t _gogc_used_at_gc0_end;
  size_t _gogc_used_at_gogc_begin;

  // Used to prevent the duty cycle from being reduced in the middle of a gogc
  // cycle.
  ...
  enum {
    _GC0_VALID = 0x1,
    _gogc_VALID = 0x2,
    _ALL_VALID = _GC0_VALID | _gogc_VALID
  };

  unsigned int _valid_bits;

 protected:
  double gogc_free_adjustment_factor(size_t free) const;
  void   adjust_gogc_free_adjustment_factor(bool fail, size_t free);

 public:
  gogcStats(goConcurrentMarkSweepGeneration* gogc_gen,
           unsigned int alpha = gogcExpAvgFactor);
  ...
  void record_gc0_end(size_t gogc_gen_bytes_used);
  void record_gogc_begin();
  void record_gogc_end();

  // Allow management of the gogc timer, which must be stopped/started around
  // yield points.
  elapsedTimer& gogc_timer()     { return _gogc_timer; }
  void start_gogc_timer()        { _gogc_timer.start(); }
  void stop_gogc_timer()         { _gogc_timer.stop(); }
  ...
  double gogc_period() const          { return _gogc_period; }
  double gogc_duration() const        { return _gogc_duration; }
  size_t gogc_allocated() const       { return _gogc_allocated; }

  size_t gogc_used_at_gc0_end() const { return _gogc_used_at_gc0_end;}

  // Seconds since the last background gogc cycle began or ended.
  double gogc_time_since_begin() const;
  double gogc_time_since_end() const;
  ...
  double gogc_allocation_rate() const;

  // Rate at which space in the gogc generation is being consumed (sum of the
  // above two).
  double gogc_consumption_rate() const;

  // Returns an estimate of the number of seconds until the gogc generation will
  // fill up, assuming no collection work is done.
  double time_until_gogc_gen_full() const;

  // Returns an estimate of the number of seconds remaining until
  // the gogc generation collection should start.
  double time_until_gogc_start() const;
...
// we embed in the gogcCollector, since we need to pass
...
// For objects in the gogc generation, this closure checks
// if the object is "live" (reachable). Used in weak
// reference processing.
class gogcIsAliveClosure: public BoolObjectClosure {
  const MemRegion  _span;
  const gogcBitMap* _bit_map;

  friend class gogcCollector;
 public:
  gogcIsAliveClosure(MemRegion span,
                    gogcBitMap* bit_map):
    _span(span),
    _bit_map(bit_map) {
...
// Implements AbstractRefProcTaskExecutor for gogc.
class gogcRefProcTaskExecutor: public AbstractRefProcTaskExecutor {
public:

  gogcRefProcTaskExecutor(gogcCollector& collector)
...
class gogcCollector: public CHeapObj<mtGC> {
  friend class VMStructs;
  friend class goConcurrentMarkSweepThread;
  friend class goConcurrentMarkSweepGeneration;
  friend class CompactibleFreeListSpace;
  friend class gogcParMarkTask;
  friend class gogcParInitialMarkTask;
  friend class gogcParRemarkTask;
  friend class gogcConcMarkingTask;
  friend class gogcRefProcTaskProxy;
  friend class gogcRefProcTaskExecutor;
  ...
  friend class gogcKeepAliveClosure;           //  -- ditto --
  friend class gogcDrainMarkingStackClosure;   //  -- ditto --
  friend class gogcInnerParMarkAndPushClosure; //  -- ditto --
  ...
  friend class VM_gogc_Operation;
  friend class VM_gogc_Initial_Mark;
  friend class VM_gogc_Final_Remark;
  friend class TracegogcMemoryManagerStats;
 ...
 private:
  ...
  gogcBitMap     _verification_mark_bm;
  ...
  // XXX Move these to gogcStats ??? FIX ME !!!
  ...
  gogcTracer* _gc_tracer_cm;
  ConcurrentGCTimer* _gc_timer_cm;

  bool _gogc_start_registered;
 ...
 protected:
  goConcurrentMarkSweepGeneration* _gogcGen;  // Old gen (gogc)
  MemRegion                      _span;    // Span covering above
  CardTableRS*                   _ct;      // Card table

  // gogc marking support structures
  gogcBitMap     _markBitMap;
  gogcBitMap     _modUnionTable;
  gogcMarkStack  _markStack;
  ...
  // the current gogc cycle and could lead to stack resizing efforts at
  ...
  gogcIsAliveClosure             _is_alive_closure;
  // Keep this textually after _markBitMap and _span; c'tor dependency.

  goConcurrentMarkSweepThread*     _gogcThread;   // The thread doing the work
  ModUnionClosurePar _modUnionClosurePar;

  // gogc abstract state machine
 ...
 protected:
  ...
  // Support for gogcScheduleRemark (abortable preclean)
  ...
  enum gogc_op_type {
    gogc_op_checkpointRootsInitial,
    gogc_op_checkpointRootsFinal
  };

  void do_gogc_operation(gogc_op_type op, GCCause::Cause gc_cause);
  bool stop_world_and_do(gogc_op_type op);

  OopTaskQueueSet* task_queues() { return _task_queues; }
  YieldingFlexibleWorkGang* conc_workers() { return _conc_workers; }

  // Support for parallelizing Eden rescan in gogc remark phase
  void sample_eden(); // ... sample Eden space top

 private:
  // Support for parallelizing young gen rescan in gogc remark phase
  ...
  bool take_from_overflow_list(size_t num, gogcMarkStack* to_stack);
 ...
 private:
  ...
  size_t preclean_mod_union_table(goConcurrentMarkSweepGeneration* old_gen,
                                  ScanMarkedObjectsAgainCarefullyClosure* cl);
  size_t preclean_card_table(goConcurrentMarkSweepGeneration* old_gen,
                             ScanMarkedObjectsAgainCarefullyClosure* cl);
  ...
  void sweepWork(goConcurrentMarkSweepGeneration* old_gen);
  ...
  void setup_gogc_unloading_and_verification_state();
 public:
  gogcCollector(goConcurrentMarkSweepGeneration* gogcGen,
               CardTableRS*                   ct);
  goConcurrentMarkSweepThread* gogcThread() { return _gogcThread; }
  ...
  goConcurrentMarkSweepGeneration* gogcGen() { return _gogcGen; }

  // Locking checks
  NOT_PRODUCT(static bool have_gogc_token();)
  ...
  gogcBitMap* markBitMap()  { return &_markBitMap; }
  void directAllocated(HeapWord* start, size_t size);

  // Main gogc steps and related support
  ...
  NOT_PRODUCT(bool is_gogc_reachable(HeapWord* addr);)
  ...
  // Accessors
  gogcMarkStack* verification_mark_stack() { return &_markStack; }
  gogcBitMap*    verification_mark_bm()    { return &_verification_mark_bm; }
...
class gogcExpansionCause : public AllStatic  {
 public:
  enum Cause {
    _no_expansion,
    _satisfy_free_ratio,
  ...
  static const char* to_string(gogcExpansionCause::Cause cause);
};

class goConcurrentMarkSweepGeneration: public CardGeneration {
  friend class VMStructs;
  friend class goConcurrentMarkSweepThread;
  friend class ConcurrentMarkSweep;
  friend class gogcCollector;
 protected:
  static gogcCollector*       _collector; // the collector that collects us
  CompactibleFreeListSpace*  _gogcSpace;  // underlying space (only one for now)

  // Performance Counters
  GenerationCounters*      _gen_counters;
  GSpaceCounters*          _space_counters;

  // Words directly allocated, used by gogcStats.
  ...
  // accessors
  void set_expansion_cause(gogcExpansionCause::Cause v) { _expansion_cause = v;}
  gogcExpansionCause::Cause expansion_cause() const { return _expansion_cause; }

  // Accessing spaces
  CompactibleSpace* space() const { return (CompactibleSpace*)_gogcSpace; }

 private:
  // For parallel young-gen GC support.
  gogcParGCThreadState** _par_gc_thread_states;

  // Reason generation was expanded
  gogcExpansionCause::Cause _expansion_cause;
  ...
  // Fraction of current occupancy at which to start a gogc collection which
 ...
 protected:
  ...
  void expand_for_gc_cause(size_t bytes, size_t expand_bytes, gogcExpansionCause::Cause cause);

  void assert_correct_size_change_locking();

 public:
  goConcurrentMarkSweepGeneration(ReservedSpace rs,
  ...
  // Accessors
  gogcCollector* collector() const { return _collector; }
  static void set_collector(gogcCollector* collector) {
    assert(_collector == NULL, "already set");
    _collector = collector;
  }
  CompactibleFreeListSpace*  gogcSpace() const { return _gogcSpace;  }
  ...
  bool refs_discovery_is_mt()     const {
    // Note: gogc does MT-discovery during the parallel-remark
  ...
  void clear_expansion_cause() { _expansion_cause = gogcExpansionCause::_no_expansion; }
  ...
  // Used by gogcStats to track direct allocation.  The value is sampled and
  ...
  HeapWord* expand_and_par_lab_allocate(gogcParGCThreadState* ps, size_t word_sz);
  ...
  // Iteration support specific to gogc generations
  ...
  virtual const char* short_name() const { return "gogc"; }
  void        print() const;
...
//
// Closures of various sorts used by gogc to accomplish its work
//
...
class MarkFromRootsClosure: public BitMapClosure {
 ...
 public:
  MarkFromRootsClosure(gogcCollector* collector, MemRegion span,
                       gogcBitMap* bitMap,
                       gogcMarkStack*  markStack,
...
class ParMarkFromRootsClosure: public BitMapClosure {
 ...
 public:
  ParMarkFromRootsClosure(gogcConcMarkingTask* task, gogcCollector* collector,
                          MemRegion span,
                          gogcBitMap* bit_map,
                          OopTaskQueue* work_queue,
                          gogcMarkStack*  overflow_stack);
  bool do_bit(size_t offset);
  inline void do_yield_check();
...
class PushAndMarkVerifyClosure: public MetadataVisitingOopIterateClosure {
  gogcCollector*    _collector;
  MemRegion        _span;
  gogcBitMap*       _verification_bm;
  gogcBitMap*       _gogc_bm;
  gogcMarkStack*    _mark_stack;
 protected:
  void do_oop(oop p);
  template <class T> void do_oop_work(T *p);

 public:
  PushAndMarkVerifyClosure(gogcCollector* gogc_collector,
                           MemRegion span,
                           gogcBitMap* verification_bm,
                           gogcBitMap* gogc_bm,
                           gogcMarkStack*  mark_stack);
  void do_oop(oop* p);
...
class MarkFromRootsVerifyClosure: public BitMapClosure {
 ...
 public:
  MarkFromRootsVerifyClosure(gogcCollector* collector, MemRegion span,
                             gogcBitMap* verification_bm,
                             gogcBitMap* gogc_bm,
                             gogcMarkStack*  mark_stack);
  bool do_bit(size_t offset);
...
class ScanMarkedObjectsAgainClosure: public UpwardsObjectClosure {
 ...
 public:
  ScanMarkedObjectsAgainClosure(gogcCollector* collector,
                                MemRegion span,
...
class MarkFromDirtyCardsClosure: public MemRegionClosure {
 ...
 public:
  MarkFromDirtyCardsClosure(gogcCollector* collector,
                            MemRegion span,
...
class ScanMarkedObjectsAgainCarefullyClosure: public ObjectClosureCareful {
 ...
 public:
  ScanMarkedObjectsAgainCarefullyClosure(gogcCollector* collector,
                                         MemRegion     span,
                                         gogcBitMap* bitMap,
                                         gogcMarkStack*  markStack,
                                         MarkRefsIntoAndScanClosure* cl,
...
class SurvivorSpacePrecleanClosure: public ObjectClosureCareful {
 ...
 public:
  SurvivorSpacePrecleanClosure(gogcCollector* collector,
                               MemRegion     span,
                               gogcBitMap*    bit_map,
                               gogcMarkStack* mark_stack,
...
class SweepClosure: public BlkClosureCareful {
  gogcCollector*                  _collector;  // collector doing the work
  goConcurrentMarkSweepGeneration* _g;    // Generation being swept
  ...
  gogcBitMap*                     _bitMap;       // Marking bit map (in
                                                // generation)
 ...
 public:
  SweepClosure(gogcCollector* collector, goConcurrentMarkSweepGeneration* g,
               gogcBitMap* bitMap, bool should_yield);
...
// During gogc' weak reference processing, this is a
...
class gogcDrainMarkingStackClosure: public VoidClosure {
  gogcCollector*        _collector;
  MemRegion            _span;
  gogcMarkStack*        _mark_stack;
  gogcBitMap*           _bit_map;
  gogcKeepAliveClosure* _keep_alive;
  bool                 _concurrent_precleaning;
 public:
  gogcDrainMarkingStackClosure(gogcCollector* collector, MemRegion span,
                      gogcBitMap* bit_map, gogcMarkStack* mark_stack,
                      gogcKeepAliveClosure* keep_alive,
                      bool cpc):
    _collector(collector),
    _span(span),
    _mark_stack(mark_stack),
...
// A parallel version of gogcDrainMarkingStackClosure above.
class gogcParDrainMarkingStackClosure: public VoidClosure {
  gogcCollector*           _collector;
  MemRegion               _span;
  OopTaskQueue*           _work_queue;
  gogcBitMap*              _bit_map;
  gogcInnerParMarkAndPushClosure _mark_and_push;

 public:
  gogcParDrainMarkingStackClosure(gogcCollector* collector,
                                 MemRegion span, gogcBitMap* bit_map,
                                 OopTaskQueue* work_queue):
    _collector(collector),
    _span(span),
    _work_queue(work_queue),
...
// precleaning work.
class gogcPrecleanRefsYieldClosure: public YieldClosure {
  gogcCollector* _collector;
  void do_yield_work();
 public:
  gogcPrecleanRefsYieldClosure(gogcCollector* collector):
    _collector(collector) {}
  virtual bool should_return();
};


// Convenience class that locks free list locks for given gogc collector
class FreelistLocker: public StackObj {
 private:
  gogcCollector* _collector;
 public:
  FreelistLocker(gogcCollector* collector):
    _collector(collector) {
...
class MarkDeadObjectsClosure: public BlkClosure {
  const gogcCollector*             _collector;
  const CompactibleFreeListSpace* _sp;
  gogcBitMap*                      _live_bit_map;
  gogcBitMap*                      _dead_bit_map;
public:
  MarkDeadObjectsClosure(const gogcCollector* collector,
                         const CompactibleFreeListSpace* sp,
                         gogcBitMap *live_bit_map,
                         gogcBitMap *dead_bit_map) :
    _collector(collector),
    _sp(sp),
...
class TracegogcMemoryManagerStats : public TraceMemoryManagerStats {

 public:
  TracegogcMemoryManagerStats(gogcCollector::CollectorState phase, GCCause::Cause cause);
};


#endif // SHARE_GC_gogc_goConcurrentMarkSweepGeneration_HPP
